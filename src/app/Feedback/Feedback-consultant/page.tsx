...
export default function FeedbackConsultantPage() {
    ...
    return (
      ...
      >
        <div className="w-full max-w-md rounded-2xl bg-white p-6 text-center shadow-sm">
          <p className="font-bold text-red-600">شناسه مشاور مشخص نشده است.</p>
        </div>
      </main>
    );
  }

  const entityId = Number(consultantId);
  ...
  if (!Number.isInteger(entityId) || entityId <= 0) {
    return (
      ...
      >
        <div className="w-full max-w-md rounded-2xl bg-white p-6 text-center shadow-sm">
          <p className="font-bold text-red-600">شناسه مشاور نامعتبر است.</p>
        </div>
      </main>
    );
  }
  ...
  return (
    <main dir="rtl" className="min-h-screen bg-slate-50 px-4 py-10">
      <div className="mx-auto w-full max-w-2xl">
        <FeedbackForm type="consultant" entityId={entityId} />
      </div>
    </main>
  );
}
