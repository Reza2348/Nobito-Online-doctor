...
interface ClinicsTypeProps {
  icon: React.ReactNode;
  label: string;
  active?: boolean;
}

const ClinicsType: React.FC<ClinicsTypeProps> = ({
  icon,
  label,
  active = false,
}) => {
...
const ClinicsSidebar: React.FC<ClinicsSidebarProps> = ({ clinicId }) => {
  ...
  const handleClick = async (item: number) => {
    ...
    try {
      ...
      if (error || !user) {
        ...
        const redirectUrl = `/Feedback/Feedback-clinics?clinicId=${encodeURIComponent(
          String(clinicId),
        )}`;
      ...
      const feedbackUrl = `/Feedback/Feedback-clinics?clinicId=${encodeURIComponent(
        String(clinicId),
      )}`;
  ...
  return (
    ...
    >
      ...
      <div
        ...
        "
      >
        <ClinicsType icon={<FaPhone size={17} />} label="تلفنی" />

        <ClinicsType icon={<FaUserFriends size={17} />} label="حضوری" active />

        <ClinicsType icon={<FaVideo size={17} />} label="آنلاین" />

        <ClinicsType icon={<FaCommentDots size={17} />} label="متنی" />
      </div>
        ...
        {[1, 2, 3].map((item) => (
          <div
            ...
            className="
              ...
              hover:border-sky-100 hover:bg-sky-50/20
              hover:shadow-md
            "
          >
                  ...
                  <>
                    <span
                      className="
                        ...
                        border-2 border-sky-200 border-t-sky-600
                      "
                    />
                    در حال ثبت...
                  </>
